...
class RewardAgent:
    ...
    def dummy_judge_different_types(self, instruction, answers, **kwargs):
        dummy_plan_result = self.planner.plan(instruction)
        dummy_result = rm.dummy_get_reward(instruction, answers, **kwargs)
        dummy_judge_res, scores = self.judger.dummy_judge(instruction, dummy_result)
        # winner = self.process_judgment(dummy_judge_res)
